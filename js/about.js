...
document.querySelectorAll('.nav-links a, .footer-section a').forEach(link => {
  link.addEventListener('click', function(e) {
    ...
    // If it's the About page link and we're already on About page
    if (href.includes('about.html') && window.location.pathname.includes('about.html')) {
      e.preventDefault();
      ...
      updateActiveNavState('about.html');
...
function updateActiveNavState(activePage) {
  document.querySelectorAll('.nav-links a').forEach(link => {
    ...
    if (link.getAttribute('href') === activePage || 
        (activePage === 'about.html' && link.getAttribute('href').includes('about.html'))) {
...
if ('IntersectionObserver' in window) {
  ...
  document.querySelectorAll('.value-item, .mv-card, .about-image, .story-image').forEach(el => {
    observer.observe(el);
  });
}
